...
try:
    ...
    Sequence = MovieSequence = SoundSequence = TextSequence = AdjustmentSequence = EffectSequence = ColorSequence = Scene = object  # type: ignore
...
class BlenderVSEEditor:
    ...
    def create_project(
        ...
        if not HAS_BPY:
            raise VSEError(
                "Blender Python API (bpy) недоступен. "
                "Запустите этот код внутри Blender (Scripting > Run Script)."
            )
        try:
    ...
    def add_video_clip(
        ...
        try:
            ...
            # --- Низкоуровневый API (работает в headless) ---
            name = f"Movie_{self._clip_counter:03d}"
            new_strip = self._se.sequences.new_movie(
                name=name,
                filepath=filepath,
                channel=channel,
    ...
    def add_text(
        ...
        try:
            # --- Низкоуровневый API (работает в headless) ---
            name = f"Text_{text[:15].replace(' ', '_')}"
            txt_strip = self._se.sequences.new_effect(
                name=name,
                type="TEXT",
                channel=channel,
                frame_start=frame_start,
    ...
    def animate_text_zoom_in(
        ...
        """
        Animation: zoom in (text grows via font_size).

        Args:
            strip:       Text strip.
            zoom_frames: Frames for zoom animation.
        """
        try:
            start = int(strip.frame_start)
            end = min(start + zoom_frames, int(strip.frame_final_end))
            base_size = strip.font_size

            # Ensure animation_data exists
            if strip.animation_data is None:
                strip.animation_data_create()

            # --- Start: small font + transparent ---
            strip.font_size = int(base_size * 0.3)
            strip.blend_alpha = 0.0
            strip.keyframe_insert(data_path="blend_alpha", frame=start)
            strip.keyframe_insert(data_path="font_size", frame=start)

            # --- End: full size + visible ---
            strip.font_size = base_size
            strip.blend_alpha = 1.0
            strip.keyframe_insert(data_path="blend_alpha", frame=end)
            strip.keyframe_insert(data_path="font_size", frame=end)
            ...
            self._set_keyframe_interpolation(
                strip, "font_size", "BACK", "EASE_OUT"
            )

            logger.info(
                "Zoom-in animation: '%s' frames %d-%d", strip.name, start, end
            )

        except Exception as exc:
            logger.error("Error animate_text_zoom_in: %s", exc)
            raise VSEError(f"Zoom-in animation failed: {exc}") from exc
    ...
    def add_color_grade(
        ...
        try:
            # --- Низкоуровневый API (работает в headless) ---
            adj = self._se.sequences.new_effect(
                name="ColorGrade",
                type="ADJUSTMENT",
                channel=channel,
                frame_start=frame_start,
            ...
            # --- Применяем параметры цветокоррекции через модификатор ---
            self._add_strip_modifier(adj, "COLOR_BALANCE", {
                "color_multiply": color_multiply[0] if isinstance(color_multiply, (list, tuple)) else color_multiply,
    ...
    def add_vignette(
        ...
        try:
            # --- Низкоуровневый API (работает в headless) ---
            vig = self._se.sequences.new_effect(
                name="Vignette",
                type="COLOR",
                channel=channel,
                frame_start=frame_start,
    ...
    def add_sound(
        ...
        try:
            ...
            # --- Низкоуровневый API (работает в headless) ---
            name = f"Sound_{self._clip_counter:03d}"
            sound = self._se.sequences.new_sound(
                name=name,
                filepath=filepath,
                channel=channel,
                frame_start=frame_start,
    ...
    def _set_keyframe_interpolation(
        ...
        if strip.animation_data is None:
            return
        if not strip.animation_data.action:
            return
