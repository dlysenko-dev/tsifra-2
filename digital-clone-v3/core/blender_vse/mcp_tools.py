...
# ---------------------------------------------------------------------------
# MCPTool dataclass — импорт из core.mcp_layer с fallback
# ---------------------------------------------------------------------------
...
try:
    ...
    _HAS_MCP_LAYER = False

    # Fallback: если core.mcp_layer недоступен — собственная реализация
...
def _handle_add_clip(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Добавляет видео- или аудио-клип на timeline.

    Args:
        params: {
            "filepath": str (required),
            "strip_type": str (default: "video", options: "video" | "sound"),
    ...
    try:
        ...
        strip_type = params.get("strip_type", "video")
...
def register_blender_vse_tools(mcp_layer) -> List[str]:
    ...
    """
    # Определяем tool_type: если импортирован из mcp_layer — используем VIDEO_GEN
    if _HAS_MCP_LAYER:
        _tt = MCPToolType.VIDEO_GEN
    else:
        _tt = None
