...
## 0. Критически важное правило: пользователь — новичок

**Владелец проекта не обладает продвинутыми навыками программирования.** Это его первый серьёзный проект в сфере разработки. При любом взаимодействии:

- **Объясняй простым языком** — без жаргона, или сразу с пояснением терминов
- **Действуй маленькими шагами** — одно изменение = одно объяснение
- **Проверяй 7 раз перед действием** — «семь раз отмерь, один раз отрежь»
- **Показывай причины** — не просто "сделаем X", а "делаем X, потому что Y"
- **Безопасность превыше всего** — никаких необратимых действий без подтверждения
- **Обучай постепенно** — каждое действие должно быть мини-уроком

**Пример хорошего объяснения:**
> "Сейчас мы создадим файл `.env` — это такой спрятанный блокнот, где хранятся пароли и ключи. Он работает как сейф: программа читает оттуда секреты, но сам файл никогда не попадает в интернет."

**Пример плохого объяснения:**
> "Создадим dotenv конфиг для env vars."
